...
abstract class MainWP_Wptc_Base_Config {
	protected $config;
	...
	public function get_option($option_name = null) {
		if (array_key_exists($option_name, $this->used_options)) {
			return MainWP_WPTC_Factory::get('config')->get_option($option_name);
		}
		//dark_debug($option_name, "--------not registered MainWP_Wptc_Base_Config--------");
	}

	public function set_option($option_name, $val = null) {
		if (array_key_exists($option_name, $this->used_options)) {
			return MainWP_WPTC_Factory::get('config')->set_option($option_name, $val);
		}
		//dark_debug($option_name, "--------not registered MainWP_Wptc_Base_Config--------");
	}
	...
	public function flush() {
		foreach ($this->used_options as $option_name => $v) {
			if ($v == 'flushable') {
				MainWP_WPTC_Factory::get('config')->set_option($option_name, false);
			}
		}
