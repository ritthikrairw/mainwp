<?php
/**
 * Uptime Robot Metabox
 *
 * Renders Metabox table content when Uptime Robot service is selected.
 *
...
if ( ! empty( $urls ) ) {
	...
	<thead>
	  <tr>
		...
		<th><?php _e( 'Ratio', 'advanced-uptime-monitor-extension'); ?></th>
	  </tr>
	</thead>
	<tbody>
	<?php foreach ( $urls as $url ) : ?>
		...
		<td>
		<?php
		if ( isset( $url->status ) && isset( $url->alltimeuptimeratio ) ) {
		...
		<?php
		if ( isset( $url->status ) && isset( $url->alltimeuptimeratio ) ) {
			echo MainWP_AUM_Settings_Page::get_instance()->render_ratio_value( $url->status, $url->alltimeuptimeratio );
		} else {
			echo 'N / A';
		}
		?>
...
<script type="text/javascript">
	...
	MainWP_AUM_Settings_Page::render_api_not_existed( 'uptimerobot' );
}
