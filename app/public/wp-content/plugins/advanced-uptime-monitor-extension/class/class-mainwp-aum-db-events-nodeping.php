<?php
/**
 * MainWP NodePing Events Database Controller
 *
 ...
 * Class MainWP_AUM_DB
 */
class MainWP_AUM_DB_Events_Nodeping extends MainWP_AUM_DB_Base {
	...
	public function get_last_events( $url_id, $lasttime = false ) {
		$where = '';
		if ( $lasttime ) {
			$where = $this->wpdb->prepare( ' AND ( check_timestamp / 1000 ) <= %d ', $lasttime );
		}
		return $this->wpdb->get_row( 'SELECT * FROM ' . $this->table_name( 'stats_nodeping' ) . ' WHERE url_id = ' . $url_id . ' ' . $where . ' ORDER BY check_timestamp DESC LIMIT 1' );
	}
	...
	public function get_events( $where, $limit = false, $order_by = '' ) {
		...
		$sql = 'SELECT * FROM ' . $this->table_name( 'stats_nodeping' ) . ' WHERE 1 ' . $where . ' ORDER BY  ' . $order_by . $limit;
	...
	public function delete_events( $id ) {
		...
		$nr = $this->wpdb->query( $this->wpdb->prepare( 'DELETE FROM ' . $this->table_name( 'stats_nodeping' ) . ' WHERE url_id=%d', $id ) );
		return $nr;
	}
	...
	public function insert_event( $data ) {
		...
		$this->wpdb->insert( $this->table_name( 'stats_nodeping' ), $data );
	...
	public function update_event( $event_id, $fields ) {
		if ( count( $fields ) > 0 ) {
			return $this->wpdb->update( $this->table_name( 'stats_nodeping' ), $fields, array( 'event_id' => $event_id ) );
		}
		return false;
	}
	...
	public function get_cond_events( $options = array() ) {
		...
		$order_by   = isset( $options['order_by'] ) ? $options['order_by'] : ' check_timestamp DESC ';
		...
		if ( $count_only ) {
			$sql = 'SELECT count(event_id) FROM ' . $this->table_name( 'stats_nodeping' ) . ' WHERE 1 ' . $where . ' ORDER BY ' . $order_by . $limit;
