...
final class MainWP_ITSEC_Network_Brute_Force_Utilities {
	...
	public static function get_api_key( $email, $optin ) {
		...
		if ( ! is_email( $email ) ) {
			return new WP_Error( 'itsec-network-brute-force-utilities-get-api-key-bad-email', sprintf( __( 'The supplied email address (%s) is invalid. A valid email address is required in order to sign up for the Network Bruteforce Protection by iThemes.', 'l10n-mainwp-ithemes-security-extension' ), $email ) );
		}
		...
		if ( ! is_array( $body ) || ! isset( $body['apikey'] ) ) {
			return new WP_Error( 'itsec-network-brute-force-utilities-get-api-key-bad-response', __( 'An unknown error prevented the API key request from succeeding. The request for an API key returned an unrecognized response. Please wait a few minutes and try again.', 'l10n-mainwp-ithemes-security-extension' ) );
		}

		$key = trim( sanitize_text_field( $body['apikey'] ) );

		if ( empty( $key ) ) {
			return new WP_Error( 'itsec-network-brute-force-utilities-get-api-key-bad-response', __( 'An unknown error prevented the API key request from succeeding. The request for an API key returned an empty key. Please wait a few minutes and try again.', 'l10n-mainwp-ithemes-security-extension' ) );
		}
	...
	public static function activate_api_key( $api_key ) {
		...
		if ( empty( $api_key ) ) {
			return new WP_Error( 'itsec-network-brute-force-utilities-activate-api-key-empty-key', __( 'An unknown error prevented the API key secret request from succeeding. The request for an API key submitted an empty key. Please wait a few minutes and try again.', 'l10n-mainwp-ithemes-security-extension' ) );
		}
		...
		if ( ! is_array( $body ) || ! isset( $body['secret'] ) ) {
			...
			if ( ! empty( $body['error'] ) && ! empty( $body['error']['message'] ) ) {
				return new WP_Error( 'itsec-network-brute-force-utilities-activate-api-key-error-response', sprintf( __( 'There was an error returned from the Network Brute Force API: %1$s', 'l10n-mainwp-ithemes-security-extension' ), $body['error']['message'] ) );
			}
			return new WP_Error( 'itsec-network-brute-force-utilities-activate-api-key-bad-response', __( 'An unknown error prevented the API key secret request from succeeding. The request for an API key secret returned an unrecognized response. Please wait a few minutes and try again.', 'l10n-mainwp-ithemes-security-extension' ) );
		}
