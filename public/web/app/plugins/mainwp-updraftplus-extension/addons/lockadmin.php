...
class MainWP_Updraft_Plus_Addon_LockAdmin {
	...
	public function __construct() {
		...
		if ( ( ! empty( $_POST['mwp_updraft_unlockadmin_session_length'] ) || ! empty( $_POST['mwp_updraft_unlockadmin_password'] )) && ! empty( $_POST['nonce'] ) ) {
	...
	public function admin_init() {
		...
		if ( ! is_a( $user, 'WP_User' ) ) {
				return; }
			$this->get_opts();
		if ( ! empty( $_POST['mwp_updraft_unlockadmin_session_length'] ) && isset( $_POST['mwp_updraft_unlockadmin_oldpassword'] ) && $_POST['mwp_updraft_unlockadmin_oldpassword'] == $this->opts['password'] ) {
				$this->old_password = $this->opts['password'];
				$this->opts['password'] = $_POST['mwp_updraft_unlockadmin_password'];
				$this->opts['support_url'] = $_POST['updraft_unlockadmin_support_url'];
				$this->opts['session_length'] = (int) $_POST['mwp_updraft_unlockadmin_session_length'];
		...
		if ( (string) $_POST['mwp_updraft_unlockadmin_password'] === $password ) {
	...
	public function debugtools_dashboard() {
			...
			<h3><?php echo __( 'Lock access to the UpdraftPlus settings page', 'mainwp-updraftplus-extension' ); ?></h3>
				...
				<form method="post" onsubmit="if (jQuery('#mwp_updraft_unlockadmin_password').val() != '') {
				...
				<input type="hidden" name="mwp_updraft_unlockadmin_oldpassword" value="<?php echo esc_attr( $this->opts['password'] ); ?>">
				<table>
					<?php
					echo $mainwp_updraftplus_admin->settings_debugrow( __( 'Password', 'mainwp-updraftplus-extension' ) . ':', '<input type="text" id="mwp_updraft_unlockadmin_password" name="mwp_updraft_unlockadmin_password" value="' . esc_attr( $this->opts['password'] ) . '" style="width:230px;">' );
					...
					echo $mainwp_updraftplus_admin->settings_debugrow( __( 'Require password again after', 'mainwp-updraftplus-extension' ) . ':', '<select name="mwp_updraft_unlockadmin_session_length" style="width:230px;">' . $session_options . '</select>' );
	...
	public function settings_page_render_abort() {
			...
			<div id="updraft-lock-area">
				...
				<form method="post">
					<input type="hidden" name="nonce" value="<?php echo wp_create_nonce( 'updraftplus-unlockadmin-nonce' ); ?>">
					<p>
						<input type="password" size="16" name="mwp_updraft_unlockadmin_password" value="">
