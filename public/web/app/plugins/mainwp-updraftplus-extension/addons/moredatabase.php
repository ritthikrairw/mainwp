	...
	public function extradb_testconnection() {
				...
				$ret['m'] = die(
					json_encode(
						array(
							...
							'm' => '<p>' . sprintf(
								__(
									'Failure: No %s was given.',
									'mainwp-updraftplus-extension'
								) . '</p>',
								__( 'user', 'mainwp-updraftplus-extension' )
							),
						)
					)
				);
					...
					json_encode(
						array(
							...
							'm' => '<p>' . sprintf(
								...
								__( 'host', 'mainwp-updraftplus-extension' )
							),
						)
					)
				);
					...
					json_encode(
						array(
							...
							'm' => '<p>' . sprintf(
								...
								__( 'database name', 'mainwp-updraftplus-extension' )
							),
						)
					)
				);
		...
		if ( empty( $siteid ) ) {
			...
			if ( is_array( $information ) && ! isset( $information['r'] ) ) {
					$information['r'] = esc_html( $_POST['row'] );
			}
