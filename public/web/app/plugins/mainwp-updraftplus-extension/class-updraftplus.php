	...
	public function get_backupable_file_entities( $include_others = true, $full_info = false ) {
		...
		if ( $full_info ) {
				$arr = array(
					'plugins' => array(
						'path'        => WP_PLUGIN_DIR,
						'description' => __( 'Plugins', 'mainwp-updraftplus-extension' ),
					),
					'themes'  => array(
						'path'        => WP_CONTENT_DIR . '/themes',
						'description' => __( 'Themes', 'mainwp-updraftplus-extension' ),
					),
					'uploads' => array(
						'path'        => $wp_upload_dir['basedir'],
						'description' => __( 'Uploads', 'mainwp-updraftplus-extension' ),
					),
				);
		} else {
				$arr = array(
		...
		if ( $include_others ) {
			if ( $full_info ) {
					$arr['others'] = array(
						'path'        => WP_CONTENT_DIR,
						'description' => __( 'Others', 'mainwp-updraftplus-extension' ),
					);
			} else {
