...
final class MainWP_ITSEC_Modules {
	/**
	 * @var MainWP_ITSEC_Modules - Static property to hold our singleton instance
	 */
	...
	protected function __construct() {
		...
		register_activation_hook( MainWP_ITSEC_Core::get_plugin_file(), array( $this, 'run_activation' ) );
		register_deactivation_hook( MainWP_ITSEC_Core::get_plugin_file(), array( $this, 'run_deactivation' ) );

		// Action triggered from another part of Security which runs when the settings page is loaded.
		add_action( 'mainwp-itsec-settings-page-init', array( $this, 'load_settings_page' ) );
		add_action( 'mainwp-itsec-logs-page-init', array( $this, 'load_settings_page' ) );
	}
	...
	public static function register_module( $slug, $path, $type = 'default-inactive' ) {
		...
		if ( ! is_dir( $path ) ) {
			trigger_error( sprintf( __( 'An attempt to register the %1$s module failed since the supplied path (%1$s) is invalid. This could indicate an invalid modification or incomplete installation of the iThemes Security plugin. Please reinstall the plugin and try again.', 'l10n-mainwp-ithemes-security-extension' ), $slug, $path ) );
			return false;
		}
		
		// if ( 'security-check' !== $slug ) { 
		...
		// 		$config = new MainWP_Module_Config( $config );
	...
	public static function set_settings( $slug, $settings ) {
		...
		if ( is_null( $settings_obj ) || ! is_callable( array( $settings_obj, 'set_all' ) ) ) {
			$error = new WP_Error( 'itsec-modules-invalid-settings-object', sprintf( __( 'Unable to find a valid settings object for %s. Settings were unable to be saved.', 'l10n-mainwp-ithemes-security-extension' ), $slug ) );
			MainWP_ITSEC_Response::add_error( $error );
	...
	public static function set_setting( $slug, $name, $value ) {
		...
		if ( is_null( $settings_obj ) || ! is_callable( array( $settings_obj, 'set_all' ) ) ) {
			trigger_error( sprintf( __( 'Unable to find a valid settings object for %s. Setting was unable to be saved.', 'l10n-mainwp-ithemes-security-extension' ), $slug ) );
			return false;
		}
	...
	public static function get_validator( $slug ) {
		...
		if ( ! isset( $self->_module_validators[ $slug ] ) ) {
			require_once( MainWP_ITSEC_Core::get_core_dir() . '/lib/validator.php' );
	...
	public static function get_active_modules() {
		...
		if (MainWP_IThemes_Security::is_manage_site()) {                                    
			if ($site_id = MainWP_IThemes_Security::get_manage_site_id()) {				
				...
				if (empty($itheme_settings)) {
					$itheme_settings = MainWP_IThemes_Security::get_itheme_settings();
					if ($itheme_settings) {
						MainWP_IThemes_Security::update_itheme_settings($itheme_settings, $site_id); // update individual by general settings
					} 
				}
			}
		} else {
		...
		$self->_active_modules = isset($itheme_settings[$field]) ? $itheme_settings[$field] : array();
		...
		if ( ! is_array( $self->_active_modules ) ) {
			...
			if (MainWP_IThemes_Security::is_manage_site()) {                                    
				if ($site_id = MainWP_IThemes_Security::get_manage_site_id()) {
					MainWP_IThemes_Security::update_itheme_settings(array($field => $self->_active_modules), $site_id);
				}
			} else {
				MainWP_IThemes_Security::update_itheme_settings(array($field => $self->_active_modules));
			}			
		}

		$default_active_modules = apply_filters( 'mainwp-itsec-default-active-modules', array_keys( $self->_default_active_modules ) );
	...
	public static function activate( $module_id ) {
		...
		if ( self::is_always_active( $module_id ) ) {
			return new WP_Error( 'itsec-modules-cannot-activate-always-active-module', sprintf( __( 'The %s module is a Core module and cannot be activated or deactivated.', 'l10n-mainwp-ithemes-security-extension' ), $module_id ) );
		}
	...
	public static function init_modules() {
		do_action( 'mainwp-itsec-register-modules' );
	}
	...
	public function run_activation() {
		...
		do_action( 'mainwp_itsec_modules_do_plugin_activation' );
	}

	public function run_deactivation() {
	

		do_action( 'mainwp_itsec_modules_do_plugin_deactivation' );
	}

	public static function run_uninstall() {


		do_action( 'mainwp_itsec_modules_do_plugin_uninstall' );
	}

	public function run_upgrade( $old_version, $new_version ) {

		do_action( 'mainwp_itsec_modules_do_plugin_upgrade', $old_version, $new_version );
	}
...
MainWP_ITSEC_Modules::get_instance();
