...
final class MainWP_Module_Config {
	...
	/**
	 * MainWP_Module_Config constructor.
	 *
	 * @param array $config
	 */
	...
	public function get_status( $for = '' ): string {
		...
		if ( ! $for ) {
			$for = MainWP_ITSEC_Core::is_pro() ? 'pro' : 'free';
		}
	 ...
	 *
	 * @param MainWP_Module_Config $with
	 *
	 * @return $this A new merged module config is returned.
	 */
	public function extend( MainWP_Module_Config $with ): self {
		$new = clone $this;
		...
		if ( $with->get_settings() ) {
			if ( ! $new->get_settings() ) {
				...
				$new->config['settings'] = MainWP_ITSEC_Lib::array_merge_recursive_distinct( $new->config['settings'], $with->get_settings() );
			}
		}
	...
	private static function transform_module_config( array $config ): array {
		$transformed = MainWP_ITSEC_Lib::resolve_schema_refs( $config );
		...
		if ( strpos( $value, '[' ) !== false ) {
			$translated = preg_replace_callback( '/\[([^\]]+)\]\(([^\)]+)\)/', function ( $matches ) {
				if ( 0 === strpos( $matches[2], 'itsec://' ) ) {
					...
					return MainWP_ITSEC_Core::get_link_for_settings_route( '/' . $path ) . $matches[1] . '</a>';
				}
