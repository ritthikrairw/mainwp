...
abstract class MainWP_ITSEC_Settings {
	protected $settings;
	
	public function __construct() {
		$this->load();
		
		add_action( 'mainwp-itsec-lib-clear-caches', array( $this, 'load' ), 0 );
	}
	...
	public function set_all( $settings ) {
		...
		if ( is_null( $validator ) ) {
			...
			if ( $validator->can_save() ) {
				...
				MainWP_ITSEC_Storage::set( $this->get_id(), $this->settings );
				$this->after_save();
		...
		MainWP_ITSEC_Response::add_errors( $retval['errors'] );
		MainWP_ITSEC_Response::add_messages( $retval['messages'] );
