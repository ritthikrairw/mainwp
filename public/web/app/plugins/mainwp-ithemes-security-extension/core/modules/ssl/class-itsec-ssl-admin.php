...
class MainWP_ITSEC_SSL_Admin {
	function run() {
		if ( 1 === MainWP_ITSEC_Modules::get_setting( 'ssl', 'frontend' ) ) {
	...
	function ssl_enable_per_content() {
		...
		wp_nonce_field( 'MainWP_ITSEC_Admin_Save', 'mainwp_itsec_admin_save_wp_nonce' );
		...
		$content .= '<label for="enable_ssl">' . __( 'Enable SSL:', 'l10n-mainwp-ithemes-security-extension' ) . '</label> ';
	...
	function save_post( $id ) {
		...
		if ( isset( $_POST['mainwp_itsec_admin_save_wp_nonce'] ) ) {

			if ( ! wp_verify_nonce( $_POST['mainwp_itsec_admin_save_wp_nonce'], 'MainWP_ITSEC_Admin_Save' ) || ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) || ( $_POST['post_type'] == 'page' && ! current_user_can( 'edit_page', $id ) ) || ( $_POST['post_type'] == 'post' && ! current_user_can( 'edit_post', $id ) ) ) {
				return $id;
			}
