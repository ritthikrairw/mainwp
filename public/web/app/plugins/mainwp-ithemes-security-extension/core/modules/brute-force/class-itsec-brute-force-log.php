 ...
 * @since      4.0
 */
final class MainWP_ITSEC_Brute_Force_Log extends MainWP_ITSEC_WP_List_Table {
	...
	function column_host( $item ) {
		...
		foreach ( $item['host'] as $host ) {
			if ( MainWP_ITSEC_Lib_IP_Tools::validate( $host ) ) {
	...
	public function get_columns() {
		...
		return array(
			...
			'user' => __( 'Username', 'l10n-mainwp-ithemes-security-extension' ),
		);
	...
	public function prepare_items() {
		...
		$items = $mainwp_itsec_logger->get_events( 'brute_force' );
