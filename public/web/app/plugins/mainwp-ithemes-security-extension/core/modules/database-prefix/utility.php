...
final class MainWP_ITSEC_Database_Prefix_Utility {
...
//
//
//		$config_file_path = MainWP_ITSEC_Lib_Config_File::get_wp_config_file_path();
//		$config = MainWP_ITSEC_Lib_File::read( $config_file_path );
//		
//		if ( is_wp_error( $config ) ) {
//			/* translators: 1: Specific error details */
//			$response['errors'][] = new WP_Error( $confix->get_error_code(), sprintf( __( 'Unable to read the <code>wp-config.php</code> file in order to update the Database Prefix. Error details as follows: %1$s', 'l10n-mainwp-ithemes-security-extension' ), $config->get_error_message() ) );
//			return $response;
//		}
//		
//		
...
//		
//		$write_result = MainWP_ITSEC_Lib_File::write( $config_file_path, $config );
//		
//		if ( is_wp_error( $write_result ) ) {
//			/* translators: 1: Specific error details */
//			$response['errors'][] = new WP_Error( $confix->get_error_code(), sprintf( __( 'Unable to update the <code>wp-config.php</code> file in order to update the Database Prefix. Error details as follows: %1$s', 'l10n-mainwp-ithemes-security-extension' ), $config->get_error_message() ) );
//			return $response;
//		}
//		
//		
...
//
//				$response['errors'][] = new WP_Error( 'itsec-database-prefix-utility-change-database-prefix-failed-table-rename', sprintf( __( 'Could not rename table %1$s. You may have to rename the table manually.', 'l10n-mainwp-ithemes-security-extension' ), $wpdb->base_prefix . $table ) );
//
//			}
//
//		}
...
//
//			$response['errors'][] = new WP_Error( 'itsec-database-prefix-utility-change-database-prefix-failed-options-update', __( 'Could not update prefix references in options table.', 'l10n-mainwp-ithemes-security-extension' ) );
//
//		}
//
...
//				if ( $result == false ) {
//
//					$response['errors'][] = new WP_Error( 'itsec-database-prefix-utility-change-database-prefix-failed-usermeta-update', __( 'Could not update prefix references in usermeta table.', 'l10n-mainwp-ithemes-security-extension' ) );
//
//				}
//
//			}
