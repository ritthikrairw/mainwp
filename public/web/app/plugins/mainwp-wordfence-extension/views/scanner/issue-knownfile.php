 ...
 */
echo MainWP_wfView::create('scanner/issue-base', array(
	...
	'summaryControls' => array(MainWP_wfView::create('scanner/issue-control-repair'), MainWP_wfView::create('scanner/issue-control-ignore', array('ignoreP' => __('Always Ignore', 'wordfence'), 'ignoreC' => __('Ignore Until File Changes', 'wordfence'))), MainWP_wfView::create('scanner/issue-control-show-details')),
	'detailPairs' => array(
		__('Filename', 'wordfence') => '<span class="wf-split-word-xs">${data.file}</span>',
		__('File Type', 'wordfence') => '{{if data.cType}}${MWP_WFAD.ucfirst(data.cType)}{{else}}' . __('Not a core, theme or plugin file', 'wordfence') . '.{{/if}}',
		__('Bad URL', 'wordfence') => array('(typeof data.badURL !== \'undefined\') && data.badURL', '${data.badURL}'),
		null,
		__('Details', 'wordfence') => '{{html longMsg}}',
	),
	'detailControls' => array(
		'{{if data.fileExists}}<a target="_blank" class="wf-btn wf-btn-default wf-btn-callout-subtle" rel="noopener noreferrer" href="${MWP_WFAD.makeViewFileLink(data.file,' . $site_id . ')}">' . __('View File', 'wordfence') . '</a>{{/if}}',
		'{{if data.canDiff}}<a target="_blank" class="wf-btn wf-btn-default wf-btn-callout-subtle" rel="noopener noreferrer" href="${MWP_WFAD.makeDiffLink(data,' . intval($site_id). ')}">' . __('View Differences', 'wordfence') . '</a>{{/if}}',
