(function($) {
	...
	LiveTrafficViewModel = function(listings, filters) {
		...
		self.reloadListings = function(options) {
			pullDownListings(options, function(listings, res) {
				...
				var newListings = [];
                                var site_id = 0;
                                if (typeof res !== undefined && res && res.site_id)
                                    site_id = res.site_id;
                                
				for (var i = 0; i < listings.length; i++) {
                                        listings[i].site_id = site_id; 
		...
		var pullDownListings = function(options, callback) {
			...
			var data = self.getCurrentQueryString(options);
                        data += '&site_id=' + encodeURIComponent(site_id);
                        console.log(data);
			MWP_WFAD.ajax('mainwp_wfc_loadLiveTraffic', data, function(response) {
		...
		self.prependListings = function(listings, response) {
                        var site_id = 0;
                        if (typeof response !== undefined && response && response.site_id)
		...
		self.whitelistWAFParamKey = function(path, paramKey, failedRules, site_id) {
			MWP_WFAD.ajax('mainwp_wfc_whitelistWAFParamKey', {
				path: path,
				paramKey: paramKey,
		...
		$(window).on('wf-live-traffic-ip-blocked', function(e, ip, site_id) {                        
			ko.utils.arrayForEach(self.listings(), function(listing) {                           
				if (listing.IP() === ip  && listing.site_id() == site_id) {
					listing.blocked(true);
				}
			});
		}).on('wf-live-traffic-ip-unblocked', function(e, ip, site_id) {
			ko.utils.arrayForEach(self.listings(), function(listing) {
				if (listing.IP() === ip && listing.site_id() == site_id) {
	...
	var ListingModel = function(data, groupBy) {
		...
		self.timeAgo = ko.pureComputed(function() {
			var serverTime = MWP_WFAD.serverMicrotime;
			return $(MWP_WFAD.showTimestamp(this.ctime(), serverTime)).text();
		}, self);
			...
			switch (self.action()) {
				...
				case 'blocked:waf':
					...
					if (typeof data === 'object') {
						var paramKey = MWP_WFAD.base64_decode(data.paramKey);
						var paramValue = MWP_WFAD.base64_decode(data.paramValue);
		...
		self.showWhoisOverlay = function() {
			...
			overlayBody.html('').css('opacity', 0);
                        var site_id = self.site_id();
			MWP_WFAD.ajax('mainwp_wfc_whois', {
				val: self.IP(),
                                site_id: site_id
			}, function(result) {
				var whoisHTML = MWP_WFAD.completeWhois(result, true, site_id);
				overlayBody.stop()
				.animate({
					opacity: 1
				}, 200)
		...
		self.showRecentTraffic = function() {
			...
			MWP_WFAD.ajax('mainwp_wfc_recentTraffic', {
				ip: self.IP(),
		...
		*/
		self.unblockIP = function() {
			MWP_WFAD.unblockIP(self.IP(), self.site_id(), function() {
				$(window).trigger('wf-live-traffic-ip-unblocked', [self.IP(), self.site_id()]);
			});
		};
		self.unblockNetwork = function() {
			MWP_WFAD.unblockNetwork(self.ipRangeID());
		};
		self.blockIP = function() {                        
			MWP_WFAD.blockIP(self.IP(), 'Manual block by administrator', self.site_id(), function() {                                
				$(window).trigger('wf-live-traffic-ip-blocked', [self.IP(), self.site_id()]);
			});
		};
                
                self.unblockIPNetwork = function (item) {
                    MWP_WFAD.unblockIPNetwork(item.IP());
                };
                self.blockIPNetwork = function (item) {
                    MWP_WFAD.blockIPNetwork(item.IP(), 'Manual block by administrator');
                };  
	...
	$(function() {
		...
		MWP_WFAD.wfLiveTraffic = new LiveTrafficViewModel();
		ko.applyBindings(MWP_WFAD.wfLiveTraffic, liveTrafficWrapper.get(0));
		liveTrafficWrapper.find('form').submit();
		MWP_WFAD.mode = 'liveTraffic';
		...
		$(window).on('scroll', function() {
			var win = $(this);
			var needsSticky = (MWP_WFAD.isSmallScreen ? (legendWrapper.offset().top < win.scrollTop() + 10) : (legendWrapper.offset().top < win.scrollTop() + adminBar.outerHeight() + 10));
			if (needsSticky) {
				...
				placeholder.removeClass('sticky');
                        ...
                        if (MWP_WFAD.networkActivity) {
                            ...
                            site_id = MWP_WFAD.site_id;
                        }                        
                        //console.log('infinite scroll: ' + site_id);
			...
			if (firstRow.length > 0 && firstRow.offset().top + firstRow.height() < win.scrollTop() + adminBar.outerHeight() + 20) {
				if (MWP_WFAD.mode != 'liveTraffic_paused') {
					MWP_WFAD.mode = 'liveTraffic_paused';
				}
			} else {
				if (MWP_WFAD.mode != 'liveTraffic') {
					MWP_WFAD.mode = 'liveTraffic';
				}
			}
			...
			if (hasScrolled && !loadingListings && currentScrollBottom >= scrollThreshold) {
				...
				hasScrolled = false;
				MWP_WFAD.wfLiveTraffic.loadNextListings(function() {
					loadingListings = false;
					MWP_WFAD.reverseLookupIPs(site_id);
				});
